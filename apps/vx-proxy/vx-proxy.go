...
func PrintVersion() {
	ix := info.Get()
	infoj, err := json.MarshalIndent(&ix, "", " ") // 加一点缩进
	if err != nil {
		fmt.Printf("遇到了错误: %v\n", err)
	}
...
func getCfg() *proxy.Cfg {
	cfg := &proxy.Cfg{
		Option: &proxy.Option{},
		Metric: utils.NewMetricCfg(*metricJob, *metricInstance, *metricAddress, time.Second*time.Duration(*metricIntervalSync)),
	}
