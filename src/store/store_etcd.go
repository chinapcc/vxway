...
func (e *EtcdStore) PutPlugin(value *metapb.Plugin) (uint64, error) {
	...
	value.UpdateAt = utils.NowWithMillisecond()
...
func (e *EtcdStore) GetProxies(limit int64, fn func(*metapb.Proxy) error) error {
	start := utils.MinAddrFormat
	end := getAddrKey(e.proxiesDir, utils.MaxAddrFormat)
	...
	for {
		...
		for _, item := range resp.Kvs {
			...
			start = utils.GetAddrNextFormat(value.Addr)
		}
