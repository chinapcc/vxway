...
func (dn *dispatchNode) httpOption() *utils.HTTPOption {
...
type dispatcher struct {
	...
	analysiser     *utils.Analysis
	store          store.Store
	httpClient     *utils.FastHTTPClient
...
func newDispatcher(cnf *Cfg, db store.Store, runner *task.Runner, jsEngineFunc func(*plugin.Engine)) *dispatcher {
	...
	rt := &dispatcher{
		...
		store:        db,
		runner:       runner,
		analysiser:   utils.NewAnalysis(tw),
		httpClient:   utils.NewFastHTTPClient(),
